...
public class PopularityTest3 {
	...
	private void setRelationshipsNasser(Connection connection) {
		...
		Person maria = pdao.getPerson("Maria").get(0);
	...
	/**
	 * D:\workspace_j2ee\Casual\data\tests\PopularityTest3.jpg
	 */
	@Test
	public void testPopularity() {
		...
		// porque o libero que possui mais relacionamentos e com maior numero de ocorrencias do que o leandro, aparece atrás do mesmo??
		assertEquals(relationships.get(0).getPerson2().getName(), "Beto");
		assertEquals(relationships.get(0).getScore(), 2.6d, delta);
		...
		assertEquals(relationships.get(2).getPerson2().getName(), "Líbero Rodrigues");
		assertEquals(relationships.get(2).getScore(), 1.5d, delta);
	}
